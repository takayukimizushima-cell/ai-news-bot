    ...
    for entry in d.entries[:20]:
              ...
              for attr in ("published_parsed", "updated_parsed"):
                            if hasattr(entry, attr) and getattr(entry, attr):
                                              published = datetime.fromtimestamp(mktime(getattr(entry, attr)), tz=timezone.utc)
                                              break
...
def build_slack_blocks(articles):
    ...
    blocks = [
              {"type": "header", "text": {"type": "plain_text", "text": f"AI News Digest ({now_jst})", "emoji": True}},
              {"type": "divider"},
    ]
    by_category = {}
    for a in articles:
              by_category.setdefault(a["category"], []).append(a)
    emojis = {"海外AI": ":earth_asia:", "国内AI": ":jp:", "AIリサーチ": ":microscope:"}
    for category, items in by_category.items():
              emoji = emojis.get(category, ":pushpin:")
        blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"*{emoji} {category}*"}})
        for item in items:
                      blocks.append({"type": "section", "text": {"type": "mrkdwn", "text": f"<{item['link']}|{item['title']}>\n_{item['source']}_ / {item['published']}"}})
                  blocks.append({"type": "divider"})
    return {"blocks": blocks, "text": f"AI News Digest - {len(articles)} articles"}


def build_no_news_message():
      now_jst = datetime.now(JST).strftime("%Y/%m/%d %H:%M")
    return {"blocks": [{"type": "section", "text": {"type": "mrkdwn", "text": f"*AI News Digest* ({now_jst})\nNo new articles in the past {HOURS_LOOKBACK} hours."}}], "text": "AI News Digest - No new articles"}
...
def post_to_slack(payload):
      if not SLACK_WEBHOOK_URL:
                ...
                sys.exit(1)
            resp = requests.post(SLACK_WEBHOOK_URL, json=payload, headers={"Content-Type": "application/json"}, timeout=30)
